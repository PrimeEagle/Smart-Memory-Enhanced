...
test('explicit named family facts are persisted as typed Relationship History evidence before profile validation', () => {
  ...
  assert.match(profiles, /upgradesGenericParent/);
  assert.match(profiles, /conflicting_family_role/);
  assert.match(profiles, /relationship_type_source: upgradesGenericParent/);
...
test('repair diagnostics are idempotent and scene boundaries retain their source', () => {
  ...
  assert.match(read('profiles.js'), /relationship_history_counts/);
  assert.match(read('profiles.js'), /reverse_pair_evidence_count/);
  ...
  assert.match(settings, /profile_relationship_quality_breakdown/);
  assert.match(settings, /profile_relationship_placeholders_dropped/);
...
test('developer tooling exposes a provider-free canonical idempotence check', () => {
  ...
  assert.match(settings, /Remaining reference categories:/);
...
test('session extraction repairs citation-only omissions once and never persists uncited candidates', () => {
  ...
  assert.match(session, /session_citation_pipeline/);
  assert.match(session, /candidate_terminal_records/);
  assert.match(session, /candidate_id/);
...
test('integrity round: secondary evidence promotes entities and canonical reconciliation runs automatically', () => {
  ...
  assert.match(ui, /subject_canonical_card_id/);
  assert.match(ui, /target_canonical_card_id/);
  assert.match(ui, /store: `epistemic:\$\{storeName\}`/);
...
test('chat-local cleanup: Forget This Chat and Fresh Start remove every chat-local store', () => {
  ...
  assert.match(settings, /for \(const memberName of freshStartCharacterNames\)/);
...
test('Fresh Start refreshes cleared personal prompt slots before updating token usage', () => {
  ...
  assert.match(freshStart, /freshStartCharacterNames/);
...
test('memory and navigation sections have distinct theme-neutral header icons', () => {
  ...
  for (const [marker, icon] of [
    ...
    ['sme_consolidate_enabled', 'fa-compress'],
...
test('scene prefilter admits strong deterministic seams locally while keeping provider batching bounded', () => {
  const scenes = read('scenes.js');
  const settings = read('settings.js');
  assert.match(scenes, /const deterministicSignals = deriveSceneContinuitySignals\(previous, current(?:, \{ candidate_seam_index: index \})?\)/);
