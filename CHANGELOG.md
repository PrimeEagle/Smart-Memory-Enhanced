...
- Long short-term-memory rebuilds now checkpoint every completed compaction
  pass. Each checkpoint persists the rolling summary and its exact source
  boundary, so recovery resumes from the saved tail rather than restarting the
  entire short-term phase. Only the provider request active at interruption
  must be retried.
...
- Memorize Chat budget and generation-control changes now have a synchronous,
  chat-scoped recovery copy in addition to SillyTavern's normal settings save.
  Resume restores the newest valid copy after a hard stop or restart instead
  of reverting to the values captured when the run began.
- Finalization status now identifies a provider without live token telemetry
  as soon as its request starts, rather than implying a streaming ETA is
  available through a Connection Profile.
...
- Memorize Chat now caps an overlong provider-generated short-term summary
  before saving it, using the same sentence-aware reduction used for injection.
  Stored and injected summaries therefore remain aligned instead of ending a
  run with a persistent short-term trim warning at the maximum available cap.
...
- Fixed a syntax error in the scene-boundary prefilter regular expression that
  prevented Smart Memory Enhanced from loading in SillyTavern.

### Tests

- Added direct JavaScript syntax verification for `scenes.js`; full automated
  verification passed with 306 unit/integration tests and 48 regression-harness
  assertions.
...
- Fixed the canonical integrity audit traversing registry store arrays rather
  than their entity records. Valid registry-backed persona and card-local
  references are no longer falsely reported as stale or wrong-scope.

### Tests

- Added regression coverage for flattening registry groups before collecting
  known canonical identities for the idempotence audit.
...
- Expanded the Developer idempotence result card with a privacy-safe summary
  of any remaining stale-reference categories, grouped by store, field, and
  reason code. This makes an attention result actionable without exposing
  memory text or record IDs.

### Tests

- Added regression coverage for stale-reference summary persistence and
  rendering.
...
- Fixed Memorize Chat failing before extraction because identity-review
  diagnostics referenced a handler-local `settings` variable before it was
  initialized.
...
- Verified that all published historical `codex/*` implementation branches are
  already incorporated into `main`. The branches remain published as an audit
  trail; no empty or duplicate merge commits were created.
...
- Made Profile generation tolerant of common local-model structured-output
  variations: Markdown/plain labelled sections, XML-like tags with spaces or
  hyphens, and an unclosed final section. Output still requires an explicit
  recognized profile label, so conversational prose cannot overwrite an
  approved profile.

### Tests

- Added profile parser coverage for labelled Markdown sections and unclosed
  recognizable tags.
...
- Completed staged persistence for Memorize Chat. Perspectives & Secrets, scene
  history, profiles, chat-local canon, and short-term compaction now join the
  catch-up transaction instead of issuing additional direct chat saves. This
  prevents the repeated `Timeout waiting for chat to save` failures observed
  during long runs.

### Tests

- Extended staged-persistence coverage across every catch-up metadata tier.

## [0.8.4] - 2026-07-19

### Fixed

- Fixed repeated `Timeout waiting for chat to save` failures during Memorize
  Chat for Chat-Local Only characters. Chat-local memory and Entity Registry
  writes now participate in the staged catch-up transaction and are committed
  once at the end of the run.

### Tests

- Extended persistence regression coverage to ensure chat-local memory and
  entity writes use the staged metadata-save path.

## [0.8.3] - 2026-07-19

### Fixed

- Fixed Memorize Chat catch-up scene processing after the structured scene
  response format was introduced. Duplicate detection now receives summary text
  rather than a result object, while saved scene records retain their structured
  character participants.

### Tests

- Added regression coverage for catch-up duplicate detection and participant
  preservation with structured scene results.
...
- Clarify Prompt Studio preset assignments with labeled rows, Global → This Chat → Character ordering, and the active character name.
- Add theme-neutral icons to Entity Registry, Continuity Checker, Prompt Studio, Configuration, and Developer section headers.

## [0.6.6] - 2026-07-17

### Added

- Expanded automated regression coverage for grounding validation, provider retry policy, staged persistence rollback, entity reconciliation and rename safeguards, review dialogs, and per-character memory policies.

## [0.6.5] - 2026-07-17

### Changed

- Redesign Prompt Presets as portable profiles containing every memory task, separately from Global, Character, and This Chat preset assignments with inheritance.
...
- Move the group-chat Character selector above Character Memory Policy, directly below the chat reset controls.
- Show the selector only in group chats, preventing a previous group selection from appearing in solo chats.
...
- Isolate pointer, mouse, and click events for both review dialogs so their actions do not close the Extensions panel.

### Changed

- Polish and title-case the Entity Registry reconciliation and grounding-review queue buttons.

## [0.5.3] - 2026-07-16

### Fixed

- Keep the Extensions panel open when identity-review dialog buttons, Escape, or Close are used.
...
- Entity Registry reconciliation results dialog, detailing matched, merged, ambiguous, and unmatched entries.
- Safe Entity Registry rename action with duplicate protection and reference updates.
...
- Reordered Token Usage so the context-usage bar sits beside its token count beneath the tier-mix bar.

## [0.3.9] - 2026-07-15

### Changed

- Split Token Usage into an absolute context-usage bar and a separate full-width tier-mix bar.

## [0.3.8] - 2026-07-15

### Fixed

- Long-term and session extraction examples now include required source indices, so models are not taught an output format that fails grounding validation.
...
- Memory rows now use a one-line, clickable preview in the narrow Extensions drawer.
- Added a wide memory review modal with full-text editing, validation details, source-message links, review navigation, and save/approve/reject/delete actions.

## [0.3.5] - 2026-07-15

### Fixed

- Token Usage bar segments now use their actual share of the full context window, leaving unused context visibly empty.

## [0.3.4] - 2026-07-15

### Fixed

- Token Usage now reads SillyTavern's active Context Size setting rather than a stale API context default.
...
- Per-connection-profile context-size setting for context-aware memory requests.

### Fixed

- First-time connection-profile compaction now summarizes oversized chats incrementally instead of sending the entire chat in one request.
...
- Added PrimeEagle's fork attribution before the preserved original README and author credit.

### Fixed

- Reset the Memorize Chat error indicator after Forget This Chat or Fresh Start completes.

## [0.2.2] - 2026-07-15

### Changed

- Updated all user-facing Smart Memory Enhanced panel and settings labels.

### Fixed

- Use explicit yellow for 1–10 Memorize Chat errors and red only at 11 or more, independent of the active SillyTavern theme.

## [0.2.1] - 2026-07-15

### Fixed

- Load Smart Memory Enhanced's own settings template instead of the original Smart Memory template when both extensions are installed.
