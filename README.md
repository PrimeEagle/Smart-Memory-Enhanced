...
[**`huihui_ai/qwen3-vl-abliterated`**](https://ollama.com/huihui_ai/qwen3-vl-abliterated) - good balance of quality and speed. Produces accurate character facts and reliably identifies open narrative threads without the reasoning overhead of Gemma 4. Can over-extract on arcs and session memory - tends to file more items than strictly necessary. Reference hardware uses `:8b-instruct` (6.1 GB). The `:30b` variant (20 GB, 24 GB+ recommended) has been reported to produce higher coverage with better understanding of emotional undercurrents and relationship dynamics - _community report, not tested on reference hardware_.
